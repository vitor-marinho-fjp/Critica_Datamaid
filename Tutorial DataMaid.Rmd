---
title: "Oficina Prática: Crítica e Imputação de Dados com DataMaid no R"
...
Tutorial disponível:[ https://github.com/vitor-marinho-fjp/Critica_Datamaid](https://github.com/vitor-marinho-fjp/Critica_Datamaid)
...
### Exemplos de erros em verificações de dados e limpeza de dados
...
![Fonte: Molly Olson ( R Ladies)](download.png)

1.  Resuma: quais informações a variável contém?

2.  Visualize: como é a distribuição da variável?

3.  Verifique: Quais problemas potenciais existem com a variável?

4.  Correção: corrija problemas operaçionais.

5.  Validar: realmente corrigimos o problema e/ou verificamos os erros nas linhas.
...
## Carregando os Dados de Exemplo IMRS - Saúde
...
A função `makeDataReport` gera um relatório de visão geral dos dados que *resume* o conteúdo de um conjunto de dados e *sinalize possíveis problemas*. Os problemas potenciais são identificados executando um conjunto de ***verificações** de **validação*** **específicas** da classe, de modo que diferentes verificações sejam realizadas em diferentes tipos de variáveis. As etapas de **verificação podem ser personalizadas** de acordo com a entrada do usuário e/ou tipo de dados da variável inserida
...
# Crítica de Dados Personalizada

Além da análise inicial, você pode criar regras personalizadas para crítica de dados.
...
Criando a função para ocorrências de valores iguais a zero
...
Esta função é projetada para contar os valores atípicos (outliers) em uma variável numérica `v` usando a abordagem do intervalo interquartil (IQR) com um fator de limiar (threshold).
...
Crie uma função que calcule estatísticas (média, mediana, etc.) para uma variável numérica, segmentadas por grupos de uma variável categórica. Isso pode ser útil para entender as diferenças entre grupos.
